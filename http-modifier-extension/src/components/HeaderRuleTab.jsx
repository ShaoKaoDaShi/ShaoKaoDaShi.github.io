...
const HeaderRuleTab = () => {
  ...
  const handleSubmit = () => {
    if (
      !formData.urlPattern ||
      !formData.headerName ||
      (formData.operation !== "remove" && !formData.headerValue)
    ) {
    ...
    if (editingId) {
      updatedRules = rules.map((r) =>
        r.id === editingId ? { ...newRule, enabled: r.enabled } : r,
      );
    } else {
              ...
              >
                <div className="p-3">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <span
                      className={`font-mono font-medium ${rule.operation === "remove" ? "text-red-600 line-through" : "text-gray-700"}`}
                    >
                      {rule.headerName}
                    </span>
